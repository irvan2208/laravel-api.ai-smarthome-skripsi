...
@foreach($vendor->vendorable->categories as $category)
  @if($category->menus->count())
    <section id="lqt-banner-title">
        <div class="lqt-banner-title lqt-title-our-menu" style="background: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('{{ $category->cover_url }}');">
            {{ $category->name }}
        </div>
    </section>
    <section id="lqt-menu">
        <div class="lqt-menu">
            <div class="container">
                <div class="lqt-menu-title text-center">
                    <h2 class="lqt-m0 lqt-fw400 lqt-fco">Don't miss</h2>
                    <h1 class="lqt-sub-title lqt-fw400">Today's Offer</h1>
                </div>
                <div class="row">
                    @php
                      $value = round($category->menus->count() / 2);
                      $chunked = $category->menus->chunk(intval($value));
                    @endphp
                    @foreach($chunked as $chunk)
                        <div class="col-md-6">
                            @foreach($chunk as $menu)
                                <div class="lqt-menu-list">
                                    <img src="{{ $menu->image_url }}" onclick="onClickImg(this)" class="menu-img">
                                    <div class="lqt-list-desc lqt-list-content">
                                        <div>
                                            <h3 class="lqt-m0">
                                                {{ $menu->name }}
                                                @if($menu->is_featured) 
                                                    <span class="label label-default lqt-label">HOT</span> 
                                                @endif
                                            </h3>
                                            <h3 class="pull-right lqt-m0">{{ CurrencyType::getPrefix($menu->currency) }} {{ $menu->price }}</h3>
                                        </div>
                                        <h4 class="lqt-m0 lqt-fw300">{{ $menu->description }}</h4>
                                    </div>
                                </div>
                            @endforeach
                        </div>
